      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        <a href="#" className="group flex items-center gap-3">
          ...
          <span className="flex flex-col leading-none">
            <span className="text-sm font-semibold text-white">Bryant Labs</span>
            <span className="text-xs text-muted">Custom Software Studio</span>
          </span>
        </a>
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            exit={{ opacity: 0, y: -12 }}
            className="glass-strong mx-4 rounded-3xl border border-white/10 p-4 md:hidden"
          >
            <nav className="flex flex-col gap-2">
              {navLinks.map((link) => (
                <a
                  ...
                  className="rounded-2xl px-4 py-3 text-sm text-white/90 transition hover:bg-white/5"
                >
                  {link.label}
                </a>
              ))}
